...
export const seedDatabase = mutation({
  ...
  handler: async (ctx) => {
    ...
    if (users.length < 3) {
      console.log('Not enough users in the database. Please ensure you have at least 3 users.');
      return {
        skipped: true,
...
async function createGroups(ctx: any, users: Array<{ _id: string }>) {
  const now = Date.now();
...
async function createOneOnOneExpenses(ctx: any, users: Array<{ _id: string }>) {
  const now = Date.now();
...
async function createSettlements(
  ...
  users: Array<{ _id: string }>,
  groups: Array<{ _id: string }>,
  oneOnOneExpenses: Array<{ _id: string; description: string }>,
  groupExpenses: Array<{ _id: string; description: string }>
) {
  const now = Date.now();
  ...
  const hotelExpense = groupExpenses.find((expense) => expense.description === 'Hotel reservation');
  ...
  const settlementDatas = [
    // Settlement for cab ride
    {
      ...
      relatedExpenseIds: cabExpense ? [cabExpense._id] : [],
      createdBy: user1._id,
    },
    // Settlement for hotel
    {
      ...
      groupId: groups[0]._id, // Weekend Trip Group
      relatedExpenseIds: hotelExpense ? [hotelExpense._id] : [],
      createdBy: user2._id,
    },
    // Settlement for office coffee
    {
      ...
      receivedByUserId: user2._id, // User2 receives
      groupId: groups[1]._id, // Office Expenses Group
      relatedExpenseIds: coffeeExpense ? [coffeeExpense._id] : [],
      createdBy: user3._id,
    },
  ];
